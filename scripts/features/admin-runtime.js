...
} from "../core/runtime-session-identity.js";
import {
  cleanSlug,
  decryptUploadedBlob,
  deriveIdentity,
...
import { createSessionIdentityProjectionStore } from "../core/session-identity-projection.js";
...
import { createWorkspaceSessionView } from "../core/workspace-session-view.js";
import {
  buildWorkspaceSiteKeyShare,
  findWorkspaceSiteKeyShare,
  mergeWorkspaceSiteKeyShares
} from "../core/workspace-site-key-records.js";
import {
  commentToneState,
...
} from "../core/workspace-action-deps.js";
import { createWorkspaceProjectionClient } from "../core/workspace-runtime-projections.js";
...
import { createWorkspaceDeepLinkController } from "./workspace-deep-links.js";
import { createWorkspaceInboxController } from "./workspace-inbox.js";
import { createWorkspaceMutationController } from "./workspace-mutations.js";
import { createWorkspaceAccountController } from "./workspace-account.js";
import { createWorkspaceShellController } from "./workspace-shell.js";
import { createWorkspacePageController } from "./workspace-page.js";
import { createWorkspaceTabsController } from "./workspace-tabs.js";
import { createWorkspaceUserLookupController } from "./workspace-user-lookup.js";
...
let workspaceSessionIdentityStore = null;
let workspaceSessionView = null;
...
workspacePublicStateStore = createPublicStateProjectionStore({
...
workspacePublicStateStore.subscribe((snapshot) => {
  ...
  if (snapshot.reason === "source") {
    if (workspaceState.session) {
      void workspaceSessionIdentityStore?.sync({
        force: false,
        reason: "workspace-public-state-source"
      });
    }
...
workspaceSessionIdentityStore = createSessionIdentityProjectionStore({
  page: "workspace",
  refreshDelayMs: () => 0,
  shouldRefresh: () => false
});
workspaceState.sessionIdentity = workspaceSessionIdentityStore.value;
workspaceSessionIdentityStore.subscribe((snapshot) => {
  workspaceState.sessionIdentity = snapshot.value;
  if (snapshot.reason === "source" && workspaceState.session && shouldSoftRefreshWorkspace()) {
...
const workspaceProjectionClient = createWorkspaceProjectionClient({
...
workspaceSessionView = createWorkspaceSessionView({
  state: workspaceState,
  tabs: workspaceTabs,
...
workspaceSiteKeys = createWorkspaceSiteKeyController({
  ...
  deps: {
    ...
    clearCachedInboxSubmissions: async ({ sitePubkey }) =>
      workspaceProjectionClient.clearInboxSubmissions({
        sitePubkey,
    ...
    persistCachedSiteKeyShares: async ({ shares }) =>
      workspaceProjectionClient.persistSiteKeyShares(shares, workspaceState.publicState),
    publishAdminKeyRequest,
    publishAdminKeyShare,
    publishSiteKeyEvent,
...
workspaceRuntime = createWorkspaceRuntime({
  ...
  deps: {
    ...
    loadCachedInboxSubmissions: async () =>
      (await workspaceProjectionClient.loadCachedInboxProjection())?.submissions || [],
    ...
    mergeSiteKeyShares: mergeWorkspaceSiteKeyShares,
    findSiteKeyShare: findWorkspaceSiteKeyShare,
    persistCachedInboxSubmissions: async ({ sitePubkey, submissions }) =>
      workspaceProjectionClient.persistInboxSubmissions(submissions, {
        sitePubkey,
        publicState: workspaceState.publicState
      }),
    persistCachedSiteKeyShares: async ({ shares }) =>
...
workspaceAccount = createWorkspaceAccountController({
  ...
  deps: {
    ...
    PASSWORD_MIN_LENGTH,
    persistCachedSiteKeyShares: ({ shares }) =>
      workspaceProjectionClient.persistSiteKeyShares(shares, workspaceState.publicState),
    publishAdminKeyShare,
    rebroadcastAccount,
    rememberAccountRotation: async () => null,
    rememberCurrentAccountSession: async () => null,
    resolveNextAvailableUsername,
  ...
  callbacks: {
    currentUser: () => workspaceSessionView.currentUser(),
    currentUserIsAdmin: () => workspaceSessionView.currentUserIsAdmin(),
...
workspacePage = createWorkspacePageController({
  ...
  callbacks: {
    ...
    renderWorkspace,
    scheduleUserLookup,
    setActiveTab: (tab) => workspaceTabs.setActiveTab(tab),
    submissionFilterSuggestions,
    syncWorkspace: (force = true) => workspaceRuntime.sync(force)
  }
});

export function startWorkspaceAdminRuntime() {
...
async function refreshWorkspace(force = false) {
  ...
  if (runtimeClient) {
    await runtimeClient.seedSession(workspaceState.session, { force: true }).catch(() => null);
  }
  if (workspaceState.session) {
    await workspaceSessionIdentityStore?.hydrate({
      force: Boolean(force),
      reason: force ? "workspace-refresh-force" : "workspace-refresh"
    }).catch(() => null);
  } else {
...
function workspaceSurfaceDeps() {
  ...
  return createWorkspaceSurfaceDeps({
    tabButtons: () => workspaceTabs.tabButtons(),
    renderTabButton: (tab) => workspaceTabs.renderTabButton(tab),
    currentUserIsAdmin: () => workspaceSessionView.currentUserIsAdmin(),
    currentUserHasInboxAccess: () => workspaceSessionView.currentUserHasInboxAccess(),
    currentUserPendingKeyRequest: () => workspaceSessionView.currentUserPendingKeyRequest(),
    currentUser: () => workspaceSessionView.currentUser(),
    currentRemovedSessionAccount: () => workspaceSessionView.currentRemovedSessionAccount(),
    currentRemovedSessionAccountMessage: () => workspaceSessionView.currentRemovedSessionAccountMessage(),
    currentStaleSessionAccount: () => workspaceSessionView.currentStaleSessionAccount(),
    currentStaleSessionMessage: (action) => workspaceSessionView.currentStaleSessionMessage(action),
    currentSessionUsernameConflict: () => workspaceSessionView.currentSessionUsernameConflict(),
    currentSessionUsernameConflictMessage: (action) => workspaceSessionView.currentSessionUsernameConflictMessage(action),
    visibleWorkspaceUsers: () => workspaceSessionView.visibleWorkspaceUsers(),
    renderKarmaSelectOptions,
    renderRoleSelectOptions,
    ...
    visibleWorkspaceEntities: () => workspaceSessionView.visibleWorkspaceEntities(),
    renderEntityManagementRail: () => renderWorkspaceEntityManagementRail(workspaceState, {
      escapeHtml,
      entityLocationSuggestions: () => workspaceSelectors.entityLocationSuggestions()
    }),
    ...
    formatWorkspaceKarma,
    resolveWorkspaceUserKarma,
    renderSiteKeyShareStatus: () => workspaceSiteKeys.renderSiteKeyShareStatus(),
...
function workspaceActionSurfaceDeps() {
  return createWorkspaceActionSurfaceDeps({
    siteKinds: SITE.nostr.kinds,
    sessionView: workspaceSessionView,
    userNeedsCurrentSiteKey,
    userHasUsernameConflict,
    resolveWorkspaceUserKarma,
    formatWorkspaceKarma,
    ...
    escapeHtml,
    escapeAttribute,
    workspaceUserStats: () => workspaceSessionView.workspaceUserStats(),
    resolveWorkspaceUser,
    ...
    deriveSubmissionReviewState,
    renderSubmissionStatusTags: renderWorkspaceSubmissionStatusTags,
    resolveEntityDisplayValue,
    describeSubmissionAttachment: describeWorkspaceSubmissionAttachment,
    renderLoadingState,
    firstTag: firstEventTag
  });
}
...
function renderUserIdentityButton(user, fallbackPubkey = user?.pubkey || "") {
  return renderWorkspaceUserIdentityButton({
    user,
    fallbackPubkey,
    viewerPubkey: workspaceState.viewer?.pubkey || "",
    safeAvatarUrl: safeWorkspaceAvatarUrl,
    shortKey,
    escapeAttribute,
    escapeHtml,
    profileInitials
...
function userNeedsCurrentSiteKey(user) {
  return workspaceUserNeedsCurrentSiteKey({
    ...
    activeSitePubkey: workspaceSiteKeys.activeSitePubkey()
  });
}

async function handleAttachmentDownload(button) {
  if (!workspaceSessionView.currentUserHasInboxAccess()) return;
  ...
  const siteKeyShare = workspaceSiteKeys.findSiteKeyShare(attachment.recipient_pubkey || submission?.latest?.recipient_pubkey || "");
  if (!siteKeyShare) {
...
async function handleUserAction(button) {
  if (!workspaceSessionView.currentUserIsAdmin()) return;
...
function shouldSoftRefreshWorkspace() {
  if (workspaceState.entityModal || workspaceState.chatModal || workspaceState.passwordRotationModal) return false;
  if (!workspaceState.session) return false;
